...
aws ecr get-login-password --region ${dst_region} | docker login --username AWS --password-stdin ${dst_id}.dkr.ecr.${dst_region}.${aws_endpoint}
aws ecr get-login-password --region ${src_region} | docker login --username AWS --password-stdin ${src_id}.dkr.ecr.${src_region}.${aws_endpoint}
...
docker pull ${fullname_src}
docker tag ${fullname_src} ${fullname_dst}
docker push ${fullname_dst}
